...
while epoch <= args.max_steps:
    ...
    for ib, batch in enumerate(train_batches):
        ...
        # Training step
        num_t,loss_ev_t, emae_t, pnorm_t, gnorm_t = gauss_train(model,batch,num_t,device=args.device,maxnorm=args.max_norm)
    ...
    if (epoch % args.validation_interval) == 0:
        ...
        for ib, batch in enumerate(valid_batches):
            num_v,loss_v,p, s, emae_v = gauss_predict(model, batch, num_v,device=args.device)
